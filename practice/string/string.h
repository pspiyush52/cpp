...
class string : public deque{
    ...
    private:
        ...
        static bool __contains(string::const_iterator& iter, string::const_iterator& end, const string& q, compare_fun comp);
        static bool __contains(string::const_iterator&& iter, string::const_iterator&& end, const string& q, compare_fun comp);
        static bool __contains(string::const_iterator& iter, string::const_iterator& end, const char* q, compare_fun comp);
        static bool __contains(string::const_iterator&& iter, string::const_iterator&& end, const char* q, compare_fun comp);
};

template<std::integral _num_tp>
inline string bstring(_num_tp num){
    string tmp;
    if (!num){
        tmp.push_back('0');
        return tmp;
...
template<std::integral _num_tp>
inline string bstring(_num_tp num, int width){
    if (width < 1)
...
template<std::integral T>
inline string to_string(T num, int width){
    string buf;
    for (; num; num /= 10)
        buf.push_front((num % 10) + '0');
    if (buf.size() < width){
...
template<std::integral T>
inline string to_string(T num, int width, char padding_char){
    string buf;
    for (; num; num /= 10)
        buf.push_front((num % 10) + '0');
    if (buf.size() < width){
...
template<std::floating_point T>
inline string to_string(T num, int prec){
    if (!prec)
