...
int main(void)
{
    ...
    */
    ...
    // The copy constructor will again be called for each object if we use range
    // based for loops like this
    for (Person p : p_arr)
        std::cout << "p : " << p << "\n\n";  // the copy constructor will be called at each iteration
    
    /*
        A variable p of type Person will be created and at each iteration of
        the range based for loop the current object will be copied to the
        created object p using the copy constructor which is why we will see
        the copy constructor calls for each iteration.
    */
